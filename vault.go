...
func readVault(opts *vaultSecretOptions) (string, *jsonVaultObject, error) {
	...
	entry := secret.Data[opts.SecretKeyName]
...
func getClientVault(opts *vaultSecretOptions) (*api.Client, error) {
	...
	logger.Infof("Creating vault client")
	client, err := api.NewClient(cfg)
	if err != nil {
		return nil, fmt.Errorf("error creating vault client: %v", err)
	}

	logger.Infof("Reading auth token %s", opts.AuthTokenPath)
	...
	logger.Infof("Authenticating with auth token")
	...
	logger.Infof("Authenticated successfully with vault server, acquired "+
		"policies %v", authResponse.Auth.Policies)
...
func getSecretVault(client *api.Client, name string) (*api.Secret, bool,
	error) {

	logger.Infof("Attempting to load secret %s from vault", name)
	...
	case secret == nil:
		logger.Infof("Secret %s not found in vault, got error %v", name,
			err)
...
func storeSecretVault(client *api.Client, secretData map[string]interface{},
	...
	if secretData[opts.SecretKeyName] != nil && !overwrite {
		return fmt.Errorf("entry %s in secret %s already exists: %v",
			opts.SecretKeyName, opts.SecretName,
			errTargetExists)
	}

	secretData[opts.SecretKeyName] = content

	logger.Infof("Attempting to update entry %s of secret %s in vault",
		opts.SecretKeyName, opts.SecretName)
	...
	logger.Infof("Updated secret %s", opts.SecretName)
