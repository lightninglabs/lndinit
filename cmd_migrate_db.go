...
func (x *migrateDBCommand) Execute(_ []string) error {
	...
	for _, prefix := range allDBPrefixes {
		...
		logger.Info("Opened source DB successfully")
		...
		// After the source db was checked, we open the destination db
		// and also make sure that the db was not already migrated.
		...
		logger.Info("Opened destination DB successfully")
		...
		// the db is not already successfully migrated.
		if x.ForceNewMigration {
			...
			if fileExists(metaDBPath) {
				os.Remove(metaDBPath)
			}

			// Moreover we delete all buckets at the target db to
			// start from a fresh state.
			//
			// NOTE: Because we do not allow any Bolt db as a
			// destination db we can be sure that there is no
			// chance the user can link is original db here and
			// we would delete the original data.
			// First read the top level buckets and then delete it
			...
			// Delete all top levelbuckets in one transaction.
			err = kvdb.Update(destDb, func(tx kvdb.RwTx) error {
		...
		srcDb.Close()
		logger.Info("We are now opening the source db in write mode " +
			"to set the tombstone marker. This might take a while " +
			"(~10 minutes for large databases) to sync the " +
			"freelist..., so please be patient it is the final " +
			"step.")
		...
		if err != nil {
			logger.Errorf("Error removing meta db: %v", err)
		}

		logger.Infof("After successful migration of %s, meta db at "+
			"%s was removed", prefix, metaDBPath)
