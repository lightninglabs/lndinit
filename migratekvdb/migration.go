...
func (m *Migrator) markMigrationComplete() error {
	return m.cfg.MetaDB.Update(func(tx *bbolt.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("failed to mark migration "+
				"complete: %w", err)
		}

		m.cfg.Logger.Infof("Migration marked as complete "+
			"at %s", getTimestamp())
...
func (m *Migrator) markVerificationComplete() error {
	return m.cfg.MetaDB.Update(func(tx *bbolt.Tx) error {
		metaBucket, err := tx.CreateBucketIfNotExists(
		...
		m.cfg.Logger.Infof("Verification marked as "+
			"complete at %s", getTimestamp())
...
func (m *Migrator) isMigrationComplete() (bool, time.Time, error) {
	var complete bool
	var timestamp time.Time
	err := m.cfg.MetaDB.View(func(tx *bbolt.Tx) error {
		bucket := tx.Bucket([]byte(migrationMetaBucket))
		if bucket == nil {
			return nil
		}

		tsBytes := bucket.Get([]byte(migrationCompleteKey))
		complete = tsBytes != nil

		// If complete we get the correct timestamp.
		if complete {
			if len(tsBytes) != 8 {
	...
	return complete, timestamp, err
}

// isVerificationComplete checks if the verification has been marked as
// complete.
func (m *Migrator) isVerificationComplete() (bool, time.Time, error) {
	var complete bool
	var timestamp time.Time
	err := m.cfg.MetaDB.View(func(tx *bbolt.Tx) error {
		bucket := tx.Bucket([]byte(verificationMetaBucket))
		if bucket == nil {
			return nil
		}
		...
		if complete {
			...
			ts := binary.BigEndian.Uint64(marker)
			timestamp = time.Unix(int64(ts), 0)
		}
...
func (m *Migrator) initMigrationState(ctx context.Context) error {
	err := m.migration.read(m.cfg.MetaDB)
	switch {
	...
	// We never wrote a migration state, which means we never came
	// to the first commit chunk.
	...
	default:
		return m.validateExistingMigrationState()
	}
}

// validateExistingMigrationState validates an existing migration state.
func (m *Migrator) validateExistingMigrationState() error {
	...
	m.cfg.Logger.Infof("Resuming migration from path %v, total "+
		"processed keys: %d",
...
func (m *Migrator) logVerificationProgress(path BucketPath) {
	...
	m.cfg.Logger.Infof("Verification progress at path %v: "+
		...
		path, totalKeys, totalBuckets,
		currentRate, percentComplete,
		elapsed.Round(time.Second))
...
func (m *Migrator) Migrate(ctx context.Context, sourceDB,
	...
	complete, completedTime, err := m.isMigrationComplete()
	if err != nil {
		return fmt.Errorf("failed to check if migration is "+
			"complete: %w", err)
	}
	if complete {
		m.cfg.Logger.Infof("Migration already completed at time: %v, "+
			"skipping db migration, use `--force` flag to force a "+
			"new migration (which will wipe all the data "+
			"from the destination database and start fresh)",
	...
	for {
		...
		if moreChunks {
			...
			// chunk size limit.
			err = m.migration.write(m.cfg.MetaDB)
			if err != nil {
		...
		m.cfg.Logger.Infof("Migration completed in %v (since "+
			"inital start)", elapsed)
...
func (m *Migrator) migrateBucket(ctx context.Context,
	...
	case false:
		...
		if sourceSeq != 0 {
			...
			m.cfg.Logger.Infof("Successfully copied sequence "+
...
func (m *Migrator) VerifyMigration(ctx context.Context,
	sourceDB, targetDB kvdb.Backend) error {

	m.startTimeVerification = time.Now()

	// The verification is also resumable so we check if a state exists.
	complete, completedAt, err := m.isVerificationComplete()
	if err != nil {
		return err
	}
	if complete {
		m.cfg.Logger.Infof("Verification already completed at "+
			"time: %v, skipping db verification, use `--force` "+
			"flag to force a new migration/verification (which "+
			"will wipe all the data from the destination database "+
			"and start fresh)", completedAt.Format(time.RFC3339))
		return nil
	}

	// Initialize the verification state so we resume in the right way.
	err = m.initVerificationState(ctx)
	if err != nil {
		return err
	}
	...
	// Make a final check and verify that the same amount of keys and
	// buckets were migrated and verified.
...
func (m *Migrator) compareDBs(ctx context.Context, srcDB,
	...
	return srcDB.View(func(srcTx kvdb.RTx) error {
		return targetDB.View(func(targetTx kvdb.RTx) error {
			...
			// We mark the verification as complete.
			err = m.markVerificationComplete()
			if err != nil {
				return err
			}
...
func (m *Migrator) walkAndCompare(ctx context.Context, srcBucket,
	...
	if m.verification.resuming {
		...
		if isResumptionPoint {
			m.cfg.Logger.Infof("Found verification "+
				"resumption point "+
				"at bucket %v with last unprocessed key: %s",
	...
	for ; sk != nil; sk, sv = srcCursor.Next() {
		...
		if m.verification.currentChunkBytes > m.cfg.ChunkSize {
			...
			err := m.verification.write(m.cfg.MetaDB)
			if err != nil {
				return err
			}
		...
		if !bytes.Equal(sv, tv) {
			return fmt.Errorf("value mismatch at path %v key %x, "+
				"source=%x target=%x", bucketPath, sk, sv, tv)
		}
...
func (m *Migrator) initVerificationState(ctx context.Context) error {
	// Read the migration state to get the total keys processed. So we
	...
	err = m.verification.read(m.cfg.MetaDB)
	switch {
	case err != nil && !errors.Is(err, errNoMetaBucket) &&
		!errors.Is(err, errNoStateFound):
	...
	case m.verification.persistedState == nil:
		// State does not exist, start fresh.
		m.cfg.Logger.Infof("Verification state not found, " +
			"starting fresh")
	...
	default:
		return m.validateExistingVerificationState(ctx)
	}

}

// validateExistingVerificationState validates an existing verification state.
func (m *Migrator) validateExistingVerificationState(ctx context.Context) error {
	if !m.verification.persistedState.CurrentBucketPath.HasPath() {
...
func (m *Migrator) logFinalStats() {
	...
	m.cfg.Logger.Infof("*** FINAL MIGRATION STATISTICS ***")
